...
const TEST_USER_ID = "c17_e2e_user";
const TEST_EMAIL = "c17-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  // Clear quiz attempts, certificates, badge awards, commitments for Course 17
  ...
  const c17 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-17")).limit(1);
  if (c17[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c17[0].id));
  }
...
test("Course 17 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 17 in the DB
    const c17Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-17"));
    assert.equal(c17Rows.length, 1, "Course 17 does not exist in DB");
    const course17 = c17Rows[0];
    const course17Id = course17.id;

    assert.equal(course17.courseCode, "ELH-17", "Course 17 code should be ELH-17");
    assert.equal(course17.level, "Applied Workplace Practice", "Course 17 level should be Applied Workplace Practice");

    // Course 17 contains 6 lessons and 8 questions
    const c17Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course17Id));
    assert.equal(c17Lessons.length, 6, "Course 17 should have exactly 6 lessons");

    const c17Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course17Id));
    assert.equal(c17Questions.length, 8, "Course 17 should have exactly 8 quiz questions");
    ...
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course17Id }),
    });
    ...
    const courseDetailRes = await fetch(`${API_BASE}/courses/${course17Id}`, { headers: HEADERS });
    ...
    // Step 2c. Attempt to update lesson progress using a mock enrollment ID -> expect 403 Forbidden
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course17Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c17Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course17Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Meet Prerequisites (Complete Course 12 and Course 16)
    ...
    assert.equal(c16Rows.length, 1, "Course 16 does not exist");
    const course16Id = c16Rows[0].id;

    // Fast-track completions in DB
    const prerequisitesToSeed = [course12Id, course16Id];
    ...
    // Step 4a. Enrolling in Course 17 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course17Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 17 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course17Id}`, { headers: HEADERS });
    ...
    for (const lesson of c17Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course17Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4, // incorrect choice
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course17Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainability Progress Tracker must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course17Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 17 completion");
