...
const TEST_USER_ID = "c28_e2e_user";
const TEST_EMAIL = "c28-e2e@ecolearn.mu";
...
test("Course 28 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 28 in the DB
    const c28Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-28"));
    assert.equal(c28Rows.length, 1, "Course 28 does not exist in DB");
    const course28 = c28Rows[0];
    const course28Id = course28.id;

    assert.equal(course28.courseCode, "ELH-28", "Course 28 code should be ELH-28");
    assert.equal(course28.level, "Applied Workplace Practice", "Course 28 level should be Applied Workplace Practice");

    const c28Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course28Id));
    assert.equal(c28Lessons.length, 6, "Course 28 should have exactly 6 lessons");

    const c28Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course28Id));
    assert.equal(c28Questions.length, 8, "Course 28 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course28Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course28Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course28Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c28Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));

    const submitResDummy = await fetch(`${API_BASE}/courses/${course28Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Partially Eligible Learner (Complete ELH-12 but NOT ELH-16)
    ...
    // Verify Course 28 is STILL blocked
    ...
    // 4. Meet Remaining Prerequisites (Complete Course 16)
    await db.insert(enrollmentsTable).values({
      userId: TEST_USER_ID,
      employeeId: employee.id,
      courseId: course16Id,
    ...
    // Step 5a. Enrolling in Course 28 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course28Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 28 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course28Id}`, { headers: HEADERS });
    ...
    for (const lesson of c28Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course28Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course28Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .where(eq(badgeDefinitionsTable.slug, "responsible-sustainability-communicator"))
      .limit(1);
    assert.ok(badge, "Badge Responsible Sustainability Communicator must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course28Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 28 completion");
