...
const TEST_USER_ID = "c22_e2e_user";
const TEST_EMAIL = "c22-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  const c22 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-22")).limit(1);
  if (c22[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c22[0].id));
  }
...
test("Course 22 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 22 in the DB
    const c22Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-22"));
    assert.equal(c22Rows.length, 1, "Course 22 does not exist in DB");
    const course22 = c22Rows[0];
    const course22Id = course22.id;

    assert.equal(course22.courseCode, "ELH-22", "Course 22 code should be ELH-22");
    assert.equal(course22.level, "Applied Workplace Practice", "Course 22 level should be Applied Workplace Practice");

    const c22Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course22Id));
    assert.equal(c22Lessons.length, 6, "Course 22 should have exactly 6 lessons");

    const c22Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course22Id));
    assert.equal(c22Questions.length, 8, "Course 22 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course22Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course22Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course22Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c22Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course22Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Partially Eligible Learner (Complete ELH-12 but NOT ELH-21)
    ...
    assert.equal(c21Rows.length, 1, "Course 21 does not exist");
    const course21Id = c21Rows[0].id;
    ...
    // Verify Course 22 is STILL blocked
    ...
    // 4. Meet Remaining Prerequisites (Complete Course 21)
    await db.insert(enrollmentsTable).values({
      userId: TEST_USER_ID,
      employeeId: employee.id,
      courseId: course21Id,
    ...
    // Step 5a. Enrolling in Course 22 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course22Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 22 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course22Id}`, { headers: HEADERS });
    ...
    for (const lesson of c22Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course22Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course22Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Green Team Coordinator must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course22Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 22 completion");
