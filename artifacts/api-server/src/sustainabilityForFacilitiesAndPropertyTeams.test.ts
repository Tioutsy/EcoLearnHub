...
const TEST_USER_ID = "c27_e2e_user";
const TEST_EMAIL = "c27-e2e@ecolearn.mu";
...
test("Course 27 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 27 in the DB
    const c27Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-27"));
    assert.equal(c27Rows.length, 1, "Course 27 does not exist in DB");
    const course27 = c27Rows[0];
    const course27Id = course27.id;

    assert.equal(course27.courseCode, "ELH-27", "Course 27 code should be ELH-27");
    assert.equal(course27.level, "Applied Workplace Practice", "Course 27 level should be Applied Workplace Practice");

    const c27Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course27Id));
    assert.equal(c27Lessons.length, 6, "Course 27 should have exactly 6 lessons");

    const c27Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course27Id));
    assert.equal(c27Questions.length, 8, "Course 27 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course27Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course27Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course27Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c27Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));

    const submitResDummy = await fetch(`${API_BASE}/courses/${course27Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // Verify Course 27 is STILL blocked
    ...
    // Step 5a. Enrolling in Course 27 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course27Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 27 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course27Id}`, { headers: HEADERS });
    ...
    for (const lesson of c27Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course27Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course27Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainable Facilities Practitioner must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course27Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 27 completion");
