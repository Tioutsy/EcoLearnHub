...
const TEST_USER_ID = "c25_e2e_user";
const TEST_EMAIL = "c25-e2e@ecolearn.mu";
...
test("Course 25 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 25 in the DB
    const c25Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-25"));
    assert.equal(c25Rows.length, 1, "Course 25 does not exist in DB");
    const course25 = c25Rows[0];
    const course25Id = course25.id;

    assert.equal(course25.courseCode, "ELH-25", "Course 25 code should be ELH-25");
    assert.equal(course25.level, "Applied Workplace Practice", "Course 25 level should be Applied Workplace Practice");

    const c25Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course25Id));
    assert.equal(c25Lessons.length, 6, "Course 25 should have exactly 6 lessons");

    const c25Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course25Id));
    assert.equal(c25Questions.length, 8, "Course 25 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course25Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course25Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course25Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c25Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));

    const submitResDummy = await fetch(`${API_BASE}/courses/${course25Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // Verify Course 25 is STILL blocked
    ...
    // Step 5a. Enrolling in Course 25 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course25Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 25 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course25Id}`, { headers: HEADERS });
    ...
    for (const lesson of c25Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course25Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course25Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainable Finance Supporter must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course25Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 25 completion");
