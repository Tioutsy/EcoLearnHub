...
const TEST_USER_ID = "c19_e2e_user";
const TEST_EMAIL = "c19-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  // Clear quiz attempts, certificates, badge awards, commitments for Course 19
  ...
  const c19 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-19")).limit(1);
  if (c19[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c19[0].id));
  }
...
test("Course 19 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 19 in the DB
    const c19Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-19"));
    assert.equal(c19Rows.length, 1, "Course 19 does not exist in DB");
    const course19 = c19Rows[0];
    const course19Id = course19.id;

    assert.equal(course19.courseCode, "ELH-19", "Course 19 code should be ELH-19");
    assert.equal(course19.level, "Applied Workplace Practice", "Course 19 level should be Applied Workplace Practice");

    // Course 19 contains 6 lessons and 8 questions
    const c19Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course19Id));
    assert.equal(c19Lessons.length, 6, "Course 19 should have exactly 6 lessons");

    const c19Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course19Id));
    assert.equal(c19Questions.length, 8, "Course 19 should have exactly 8 quiz questions");
    ...
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course19Id }),
    });
    ...
    const courseDetailRes = await fetch(`${API_BASE}/courses/${course19Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course19Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c19Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course19Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Partially Eligible Learner (Complete ELH-12 but NOT ELH-18)
    ...
    const c18Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-18"));
    assert.equal(c18Rows.length, 1, "Course 18 does not exist");
    const course18Id = c18Rows[0].id;
    ...
    // Verify Course 19 is STILL blocked
    ...
    // 4. Meet Remaining Prerequisites (Complete Course 18)
    await db.insert(enrollmentsTable).values({
      userId: TEST_USER_ID,
      employeeId: employee.id,
      courseId: course18Id,
    ...
    // Step 5a. Enrolling in Course 19 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course19Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 19 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course19Id}`, { headers: HEADERS });
    ...
    for (const lesson of c19Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course19Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4, // incorrect choice
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course19Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainability Performance Reviewer must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course19Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 19 completion");
