...
const TEST_USER_ID = "c24_e2e_user";
const TEST_EMAIL = "c24-e2e@ecolearn.mu";
...
test("Course 24 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 24 in the DB
    const c24Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-24"));
    assert.equal(c24Rows.length, 1, "Course 24 does not exist in DB");
    const course24 = c24Rows[0];
    const course24Id = course24.id;

    assert.equal(course24.courseCode, "ELH-24", "Course 24 code should be ELH-24");
    assert.equal(course24.level, "Applied Workplace Practice", "Course 24 level should be Applied Workplace Practice");

    const c24Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course24Id));
    assert.equal(c24Lessons.length, 6, "Course 24 should have exactly 6 lessons");

    const c24Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course24Id));
    assert.equal(c24Questions.length, 8, "Course 24 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course24Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course24Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course24Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c24Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));

    const submitResDummy = await fetch(`${API_BASE}/courses/${course24Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // Verify Course 24 is STILL blocked
    ...
    // Step 5a. Enrolling in Course 24 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course24Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 24 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course24Id}`, { headers: HEADERS });
    ...
    for (const lesson of c24Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course24Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course24Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainability-Aware HR must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course24Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 24 completion");
