...
const TEST_USER_ID = "c23_e2e_user";
const TEST_EMAIL = "c23-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  const c23 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-23")).limit(1);
  if (c23[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c23[0].id));
  }
...
test("Course 23 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 23 in the DB
    const c23Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-23"));
    assert.equal(c23Rows.length, 1, "Course 23 does not exist in DB");
    const course23 = c23Rows[0];
    const course23Id = course23.id;

    assert.equal(course23.courseCode, "ELH-23", "Course 23 code should be ELH-23");
    assert.equal(course23.level, "Applied Workplace Practice", "Course 23 level should be Applied Workplace Practice");

    const c23Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course23Id));
    assert.equal(c23Lessons.length, 6, "Course 23 should have exactly 6 lessons");

    const c23Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course23Id));
    assert.equal(c23Questions.length, 8, "Course 23 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course23Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course23Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course23Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c23Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course23Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Partially Eligible Learner (Complete ELH-12 but NOT ELH-22)
    ...
    const c22Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-22"));
    assert.equal(c22Rows.length, 1, "Course 22 does not exist");
    const course22Id = c22Rows[0].id;
    ...
    // Verify Course 23 is STILL blocked
    ...
    // 4. Meet Remaining Prerequisites (Complete Course 22)
    await db.insert(enrollmentsTable).values({
      userId: TEST_USER_ID,
      employeeId: employee.id,
      courseId: course22Id,
    ...
    // Step 5a. Enrolling in Course 23 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course23Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 23 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course23Id}`, { headers: HEADERS });
    ...
    for (const lesson of c23Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course23Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course23Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Workplace Initiative Coordinator must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course23Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 23 completion");
