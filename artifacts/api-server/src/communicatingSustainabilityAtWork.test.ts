...
const TEST_USER_ID = "c16_e2e_user";
const TEST_EMAIL = "c16-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  // Clear quiz attempts, certificates, badge awards, commitments for Course 16
  ...
  const c16 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-16")).limit(1);
  if (c16[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c16[0].id));
  }
...
test("Course 16 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 16 in the DB
    const c16Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-16"));
    assert.equal(c16Rows.length, 1, "Course 16 does not exist in DB");
    const course16 = c16Rows[0];
    const course16Id = course16.id;

    assert.equal(course16.courseCode, "ELH-16", "Course 16 code should be ELH-16");
    assert.equal(course16.level, "Applied Workplace Practice", "Course 16 level should be Applied Workplace Practice");

    // Course 16 contains 6 lessons and 8 questions
    const c16Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course16Id));
    assert.equal(c16Lessons.length, 6, "Course 16 should have exactly 6 lessons");

    const c16Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course16Id));
    assert.equal(c16Questions.length, 8, "Course 16 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    // Step 2a. Attempt to enroll directly -> expect 403 Forbidden
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course16Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    // Step 2b. Attempt to retrieve course details -> expect safe lessons (no content / blocks)
    const courseDetailRes = await fetch(`${API_BASE}/courses/${course16Id}`, { headers: HEADERS });
    ...
    // Step 2c. Attempt to update lesson progress using a mock enrollment ID -> expect 403 Forbidden or 404
    // We create a mock enrollment directly in DB to check that the prerequisite eligibility middleware locks progress update
    ...
    assert.equal(progressData.error, "PREREQUISITES_INCOMPLETE", "Error code must be PREREQUISITES_INCOMPLETE");

    // Delete mock enrollment
    await db.delete(enrollmentsTable).where(eq(enrollmentsTable.id, mockEnrollment.id));

    // Step 2d. Attempt to retrieve quiz -> expect 403 Forbidden
    const quizRes = await fetch(`${API_BASE}/courses/${course16Id}/quiz`, { headers: HEADERS });
    ...
    // Step 2e. Attempt to submit quiz -> expect 403 Forbidden
    const answersDummy = c16Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course16Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Meet Prerequisites (Complete Course 12 and Course 15)
    ...
    const c15Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-15"));
    assert.equal(c15Rows.length, 1, "Course 15 does not exist");
    const course15Id = c15Rows[0].id;

    // Fast-track completions in DB
    const prerequisitesToSeed = [course12Id, course15Id];
    for (const cid of prerequisitesToSeed) {
    ...
    // 4. Eligible Learner Access
    // Step 4a. Enrolling in Course 16 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course16Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 16 enrollment after meeting prerequisites");
    const enrollData = (await enrollRes.json()) as any;
    const enrollmentId = enrollData.id;

    // Step 4b. Fetch course detail as eligible -> verify lessons content is populated
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course16Id}`, { headers: HEADERS });
    ...
    // Step 4c. Update lesson progress for all 6 lessons -> expect 200 OK
    for (const lesson of c16Lessons) {
    ...
    // 5. Quiz Submission & Badge Verification
    // Step 5a. Submit failing quiz attempt (0%) -> expect passed: false, and no badge award
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course16Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4, // incorrect choice
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course16Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .where(eq(badgeDefinitionsTable.slug, "sustainability-communicator"))
      .limit(1);
    assert.ok(badge, "Badge Sustainability Communicator must exist in DB");
    ...
    // Step 5b. Submit passing quiz attempt (100%) -> expect passed: true, and badge awarded exactly once
    const answersPass = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: q.correctOption,
    }));

    submitRes = await fetch(`${API_BASE}/courses/${course16Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 16 completion");
    assert.equal(badgeAwards[0].employeeId, employee.id, "Badge award must belong to the correct employee");

    // Step 5c. Submit passing quiz attempt a second time -> expect passed: true, and badge is not duplicated
    submitRes = await fetch(`${API_BASE}/courses/${course16Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ answers: answersPass }),
