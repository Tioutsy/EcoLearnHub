...
const TEST_USER_ID = "c18_e2e_user";
const TEST_EMAIL = "c18-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  // Clear quiz attempts, certificates, badge awards, commitments for Course 18
  ...
  const c18 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-18")).limit(1);
  if (c18[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c18[0].id));
  }
...
test("Course 18 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 18 in the DB
    const c18Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-18"));
    assert.equal(c18Rows.length, 1, "Course 18 does not exist in DB");
    const course18 = c18Rows[0];
    const course18Id = course18.id;

    assert.equal(course18.courseCode, "ELH-18", "Course 18 code should be ELH-18");
    assert.equal(course18.level, "Applied Workplace Practice", "Course 18 level should be Applied Workplace Practice");

    // Course 18 contains 6 lessons and 8 questions
    const c18Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course18Id));
    assert.equal(c18Lessons.length, 6, "Course 18 should have exactly 6 lessons");

    const c18Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course18Id));
    assert.equal(c18Questions.length, 8, "Course 18 should have exactly 8 quiz questions");
    ...
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course18Id }),
    });
    ...
    const courseDetailRes = await fetch(`${API_BASE}/courses/${course18Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course18Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c18Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course18Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // Verify Course 18 is STILL blocked
    ...
    // Step 5a. Enrolling in Course 18 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course18Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 18 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course18Id}`, { headers: HEADERS });
    ...
    for (const lesson of c18Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course18Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4, // incorrect choice
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course18Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .limit(1);
    assert.ok(badge, "Badge Sustainability Evidence Contributor must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course18Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 18 completion");
