...
describe("Sprint 11E — ELH-05 Sustainable Procurement Assurance & Regression Suite", () => {
  test("1. Course ELH-05 exists with correct metadata and learning objectives", async () => {
    ...
    assert.ok(course, "ELH-05 course record must exist");
  ...
  test("2. ELH-05 lessons contain required interactive scenarios and anti-bribery governance blocks", async () => {
    ...
    assert.equal(lessons.length, 6, "ELH-05 must contain 6 lessons");
    ...
    assert.ok(totalScenarios >= 3, "ELH-05 must contain at least 3 decision scenarios");
  });

  test("3. ELH-05 quiz questions exhibit zero 100% Position 1 correct-answer bias", async () => {
    ...
    assert.equal(questions.length, 5, "ELH-05 quiz must contain 5 scored questions");
