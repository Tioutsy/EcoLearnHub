...
test("Course 9 (ID 9) Seeding & Integrity Unit Tests", async () => {
  ...
  const [seedMarker] = await db
    ...
    .where(eq(systemSeedsTable.name, "esg-basics-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker esg-basics-v2 must be recorded");
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "esg-basics-v2"));
