...
// Thorough cleanup of Course 25 data, recommendations, and orphans
async function cleanUpCourse25() {
  const [emp] = await db.select().from(employeesTable).where(eq(employeesTable.clerkUserId, "preserve_finance_user_id")).limit(1);
  if (emp) {
  ...
  await db.delete(quizAttemptsTable).where(eq(quizAttemptsTable.userId, "preserve_finance_user_id"));
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "sustainability-for-finance-teams-v1"));

  // Reset Course 24 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-24"));

  const c25s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-25"));
  const c25Ids = c25s.map(c => c.id);
  if (c25Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c25Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c25Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c25Ids));
  }
}

test("Course 25 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse25();
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      let c24 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-24")
      });
      if (!c24) {
        [c24] = await tx.insert(coursesTable).values({
          courseCode: "ELH-24",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 24",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 25 must be created");
      assert.equal(course.slug, "sustainability-for-finance-teams");
      assert.equal(course.title, "Sustainability for Finance Teams");
      ...
      const [badge] = await tx
        ...
        .where(eq(badgeDefinitionsTable.slug, "sustainable-finance-supporter"))
        .limit(1);
      assert.ok(badge, "Badge must be created");
      assert.equal(badge.code, "COURSE_ELH_25_COMPLETE");
      ...
      // Verify Course 24 recommendation link points to Course 25
      const [c24Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c24.id))
        .limit(1);
      assert.equal(c24Post.recommendedNextCourseId, course.id, "Course 24 should recommend Course 25");
...
test("Course 25 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse25();
  ...
  try {
    ...
    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-25")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "sustainable-finance-supporter")).limit(1);

    const [employee] = await db.insert(employeesTable).values({
      name: "Test Preserved Finance User",
      email: "preserve25@ecolearn.mu",
      clerkUserId: "preserve_finance_user_id",
      companyId: 1,
      role: "employee",
      enrolledCourses: 1,
      completedCourses: 1,
    ...
    const [enrollment] = await db.insert(enrollmentsTable).values({
      userId: "preserve_finance_user_id",
    ...
    const attemptPost = await db.query.quizAttemptsTable.findFirst({
      where: eq(quizAttemptsTable.userId, "preserve_finance_user_id")
    });
...
test("Course 25 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse25();
  ...
  try {
    const mockTx = async (callback: any) => {
      return originalTransaction.call(db, async (tx: any) => {
        await callback(tx);
        throw new Error("Forced transaction failure for testing Course 25 rollback");
      });
    };
    ...
    assert.equal(err.message, "Forced transaction failure for testing Course 25 rollback");
    didThrow = true;
  } finally {
  ...
  assert.ok(!course, "Course 25 record should not exist because the transaction rolled back");
});
