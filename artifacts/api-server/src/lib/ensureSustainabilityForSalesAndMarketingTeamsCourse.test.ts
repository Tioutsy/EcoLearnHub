...
// Thorough cleanup of Course 28 data, recommendations, and orphans
async function cleanUpCourse28() {
  const [emp] = await db.select().from(employeesTable).where(eq(employeesTable.clerkUserId, "preserve_sales_user_id")).limit(1);
  if (emp) {
  ...
  await db.delete(quizAttemptsTable).where(eq(quizAttemptsTable.userId, "preserve_sales_user_id"));
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "sustainability-for-sales-and-marketing-teams-v1"));

  // Reset Course 27 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-27"));

  const c28s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-28"));
  const c28Ids = c28s.map(c => c.id);
  if (c28Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c28Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c28Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c28Ids));
  }
}

test("Course 28 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse28();
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      let c27 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-27")
      });
      if (!c27) {
        [c27] = await tx.insert(coursesTable).values({
          courseCode: "ELH-27",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 27",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 28 must be created");
      assert.equal(course.slug, "sustainability-for-sales-and-marketing-teams");
      assert.equal(course.title, "Sustainability for Sales and Marketing Teams");
      ...
      assert.equal(badge.code, "COURSE_ELH_28_COMPLETE");
      ...
      // Verify Course 27 recommendation link points to Course 28
      const [c27Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c27.id))
        .limit(1);
      assert.equal(c27Post.recommendedNextCourseId, course.id, "Course 27 should recommend Course 28");
...
test("Course 28 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse28();
  ...
  try {
    ...
    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-28")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "responsible-sustainability-communicator")).limit(1);

    const [employee] = await db.insert(employeesTable).values({
      name: "Test Preserved Sales User",
      email: "preserve28@ecolearn.mu",
      clerkUserId: "preserve_sales_user_id",
      companyId: 1,
      role: "employee",
      enrolledCourses: 1,
      completedCourses: 1,
    ...
    const attemptPost = await db.query.quizAttemptsTable.findFirst({
      where: eq(quizAttemptsTable.userId, "preserve_sales_user_id")
    });
...
test("Course 28 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse28();
  ...
  try {
    const mockTx = async (callback: any) => {
      return originalTransaction.call(db, async (tx: any) => {
        await callback(tx);
        throw new Error("Forced transaction failure for testing Course 28 rollback");
      });
    };
    ...
    assert.equal(err.message, "Forced transaction failure for testing Course 28 rollback");
    didThrow = true;
  } finally {
  ...
  assert.ok(!course, "Course 28 record should not exist because the transaction rolled back");
});
