...
describe("Sprint 11H — ELH-08 Biodiversity Assurance & Regression Suite", () => {
  test("1. Course ELH-08 exists with correct metadata and learning objectives", async () => {
    ...
    assert.ok(course, "ELH-08 course record must exist");
  ...
  test("2. ELH-08 lessons contain required interactive scenarios and species classification blocks", async () => {
    ...
    assert.equal(lessons.length, 6, "ELH-08 must contain 6 lessons");
    ...
    assert.ok(totalScenarios >= 3, "ELH-08 must contain at least 3 decision scenarios");
  });

  test("3. ELH-08 quiz questions exhibit zero 100% Position 1 correct-answer bias", async () => {
    ...
    assert.equal(questions.length, 5, "ELH-08 quiz must contain 5 scored questions");
