...
// Thorough cleanup of Course 18 data, recommendations, and orphans
async function cleanUpCourse18() {
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "sustainability-data-collection-and-evidence-v1"));

  // Reset Course 17 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-17"));

  const c18s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-18"));
  const c18Ids = c18s.map(c => c.id);
  if (c18Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c18Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c18Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c18Ids));
  }
}

test("Course 18 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse18();

  try {
    await db.transaction(async (tx) => {
      // 0. Set up prerequisite courses (ELH-12 and ELH-17) if they do not exist
      ...
      assert.ok(course, "Course 18 must be created");
      ...
      assert.equal(badge.code, "COURSE_ELH_18_COMPLETE");

      // Verify prerequisites (ELH-12 and ELH-17)
      ...
      assert.ok(prereqIds.includes(c17.id), "Prerequisite should include Course 17");

      // Verify Course 17 recommendation link points to Course 18
      const [c17Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c17.id))
        .limit(1);
      assert.equal(c17Post.recommendedNextCourseId, course.id, "Course 17 should recommend Course 18");
...
test("Course 18 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse18();
  ...
  try {
    ...
    let c17 = await db.query.coursesTable.findFirst({ where: eq(coursesTable.courseCode, "ELH-17") });
    if (!c17) {
      [c17] = await db.insert(coursesTable).values({
        courseCode: "ELH-17",
        slug: "tracking-sustainability-actions-and-progress",
        title: "Tracking Sustainability Actions and Progress",
        level: "Applied Workplace Practice",
    ...
    // 1. Run seeder initially to create Course 18
    await ensureSustainabilityDataCollectionCourse();

    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-18")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "sustainability-evidence-contributor")).limit(1);
...
test("Course 18 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse18();
  ...
  // 4. Verify that Course 18 is not left partially seeded in the physical DB
  const course = await db.query.coursesTable.findFirst({
    where: eq(coursesTable.courseCode, "ELH-18")
  });
  assert.ok(!course, "Course 18 record should not exist because the transaction rolled back");
});
