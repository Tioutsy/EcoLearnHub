...
test("Course 11 (ID 11) Seeding & Integrity Unit Tests", async () => {
  ...
  const [course] = await db
    .select()
    .from(coursesTable)
    .where(eq(coursesTable.id, 11))
    .limit(1);

  assert.ok(course, "Course 11 must exist");
  assert.equal(course.slug, "circular-economy");
  assert.equal(course.passingScore, 80);
  assert.equal(course.status, "published");
  assert.equal(course.title, "Circular Economy");

  // Verify exactly 6 lessons
  const lessons = await db
    .select()
    .from(lessonsTable)
    .where(eq(lessonsTable.courseId, 11));
  ...
  const quizQuestions = await db
    ...
    .where(eq(quizQuestionsTable.courseId, 11));
  assert.equal(
  ...
  const [badge] = await db
    ...
    .limit(1);
  assert.ok(badge, "Circular Workplace Practitioner badge must exist");
  assert.equal(badge.name, "Circular Workplace Practitioner");

  // Verify system seed marker was recorded
  const [seedMarker] = await db
    .select()
    .from(systemSeedsTable)
    .where(eq(systemSeedsTable.name, "circular-economy-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker circular-economy-v2 must be recorded");
  ...
  await db.delete(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, 11));
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "circular-economy-v2"));
