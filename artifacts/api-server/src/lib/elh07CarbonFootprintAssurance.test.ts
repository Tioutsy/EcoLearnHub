...
describe("Sprint 11G — ELH-07 Carbon Footprint Assurance & Regression Suite", () => {
  test("1. Course ELH-07 exists with correct metadata and learning objectives", async () => {
    ...
    assert.ok(course, "ELH-07 course record must exist");
  ...
  test("2. ELH-07 lessons contain required interactive scenarios and Scope 1-3 boundaries", async () => {
    const [course] = await db
    ...
    assert.equal(lessons.length, 6, "ELH-07 must contain 6 lessons");
    ...
    assert.ok(totalScenarios >= 3, "ELH-07 must contain at least 3 decision scenarios");
  });

  test("3. ELH-07 quiz questions exhibit zero 100% Position 1 correct-answer bias", async () => {
    const [course] = await db
      .select({ id: coursesTable.id })
      .from(coursesTable)
      .where(eq(coursesTable.courseCode, "ELH-07"))
    ...
    assert.equal(questions.length, 5, "ELH-07 quiz must contain 5 scored questions");
