  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (!existingCourse) {
        ...
        logger.info(`Inserted new ELH-24 course record (ID: ${actualCourseId}).`);
      } else {
        ...
        logger.info(`Updated existing ELH-24 course record (ID: ${actualCourseId}).`);
      }
