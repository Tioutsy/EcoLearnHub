...
// Thorough cleanup of Course 19 data, recommendations, and orphans
async function cleanUpCourse19() {
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "sustainability-performance-review-v1"));

  // Reset Course 18 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-18"));

  const c19s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-19"));
  const c19Ids = c19s.map(c => c.id);
  if (c19Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c19Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c19Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c19Ids));
  }
}

test("Course 19 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse19();

  try {
    await db.transaction(async (tx) => {
      // 0. Set up prerequisite courses (ELH-12 and ELH-18) if they do not exist
      ...
      let c18 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-18")
      });
      if (!c18) {
        [c18] = await tx.insert(coursesTable).values({
          courseCode: "ELH-18",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 18",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 19 must be created");
      ...
      assert.equal(badge.code, "COURSE_ELH_19_COMPLETE");

      // Verify prerequisites (ELH-12 and ELH-18)
      ...
      assert.ok(prereqIds.includes(c18.id), "Prerequisite should include Course 18");

      // Verify Course 18 recommendation link points to Course 19
      const [c18Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c18.id))
        .limit(1);
      assert.equal(c18Post.recommendedNextCourseId, course.id, "Course 18 should recommend Course 19");
...
test("Course 19 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse19();
  ...
  try {
    ...
    let c18 = await db.query.coursesTable.findFirst({ where: eq(coursesTable.courseCode, "ELH-18") });
    if (!c18) {
      [c18] = await db.insert(coursesTable).values({
        courseCode: "ELH-18",
        slug: "sustainability-data-collection-and-evidence",
        title: "Sustainability Data Collection and Evidence",
        level: "Applied Workplace Practice",
    ...
    // 1. Run seeder initially to create Course 19
    await ensureSustainabilityPerformanceReviewCourse();

    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-19")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "sustainability-performance-reviewer")).limit(1);
...
test("Course 19 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse19();
  ...
  // 4. Verify that Course 19 is not left partially seeded in the physical DB
  const course = await db.query.coursesTable.findFirst({
    where: eq(coursesTable.courseCode, "ELH-19")
  });
  assert.ok(!course, "Course 19 record should not exist because the transaction rolled back");
});
