...
test("Course 22 (ELH-22) Seeding & Integrity Unit Tests", async () => {
  // 1. Initial seed execution
  console.log("- Running Course 22 seeder...");
  ...
  assert.ok(course, "Course ELH-22 must exist");
  ...
  const [badge] = await db
    ...
    .where(eq(badgeDefinitionsTable.slug, "effective-green-team-contributor"))
    .limit(1);
  assert.ok(badge, "Effective Green Team Contributor badge must exist");
  assert.equal(badge.name, "Effective Green Team Contributor");

  // Verify system seed marker was recorded
  const [seedMarker] = await db
    .select()
    .from(systemSeedsTable)
    .where(eq(systemSeedsTable.name, "effective-green-teams-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker effective-green-teams-v2 must be recorded");
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "effective-green-teams-v2"));
