...
test("Course 16 (ELH-16) Seeding & Integrity Unit Tests", async () => {
  // 1. Initial seed execution
  console.log("- Running Course 16 seeder...");
  ...
  assert.ok(course, "Course ELH-16 must exist");
  ...
  const [badge] = await db
    ...
    .limit(1);
  assert.ok(badge, "Credible Sustainability Communicator badge must exist");
  assert.equal(badge.name, "Credible Sustainability Communicator");

  // Verify system seed marker was recorded
  const [seedMarker] = await db
    .select()
    .from(systemSeedsTable)
    .where(eq(systemSeedsTable.name, "communicating-sustainability-at-work-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker communicating-sustainability-at-work-v2 must be recorded");
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "communicating-sustainability-at-work-v2"));
