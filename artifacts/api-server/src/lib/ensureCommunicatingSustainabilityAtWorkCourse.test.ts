...
// Thorough cleanup of Course 16 data, recommendations, and orphans
async function cleanUpCourse16() {
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "communicating-sustainability-at-work-v1"));

  // Reset Course 15 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-15"));

  const c16s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-16"));
  const c16Ids = c16s.map(c => c.id);
  if (c16Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c16Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c16Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c16Ids));
  }
}

test("Course 16 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse16();

  try {
    await db.transaction(async (tx) => {
      // 0. Set up prerequisite courses (ELH-12 and ELH-15) if they do not exist
      ...
      let c15 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-15")
      });
      if (!c15) {
        [c15] = await tx.insert(coursesTable).values({
          courseCode: "ELH-15",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 15",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 16 must be created");
      assert.equal(course.slug, "communicating-sustainability-at-work");
      assert.equal(course.title, "Communicating Sustainability at Work");
      ...
      assert.equal(badge.code, "COURSE_ELH_16_COMPLETE");

      // Verify prerequisites (ELH-12 and ELH-15)
      ...
      assert.ok(prereqIds.includes(c15.id), "Prerequisite should include Course 15");

      // Verify Course 15 recommendation link points to Course 16
      const [c15Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c15.id))
        .limit(1);
      assert.equal(c15Post.recommendedNextCourseId, course.id, "Course 15 should recommend Course 16");
...
test("Course 16 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse16();
  ...
  try {
    ...
    let c15 = await db.query.coursesTable.findFirst({ where: eq(coursesTable.courseCode, "ELH-15") });
    if (!c15) {
      [c15] = await db.insert(coursesTable).values({
        courseCode: "ELH-15",
        slug: "building-workplace-sustainability-team",
        title: "Building a Workplace Sustainability Team",
        level: "Applied Workplace Practice",
    ...
    // 1. Run seeder initially to create Course 16
    await ensureCommunicatingSustainabilityAtWorkCourse();

    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-16")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "sustainability-communicator")).limit(1);
...
test("Course 16 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse16();
  ...
  // 4. Verify that Course 16 is not left partially seeded in the physical DB
  const course = await db.query.coursesTable.findFirst({
    where: eq(coursesTable.courseCode, "ELH-16")
  });
  assert.ok(!course, "Course 16 record should not exist because the transaction rolled back");
});
