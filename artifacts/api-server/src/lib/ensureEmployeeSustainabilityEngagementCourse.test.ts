...
test("Course 21 (ELH-21) Seeding & Integrity Unit Tests", async () => {
  // 1. Initial seed execution
  console.log("- Running Course 21 seeder...");
  ...
  assert.ok(course, "Course ELH-21 must exist");
  ...
  const [badge] = await db
    ...
    .where(eq(badgeDefinitionsTable.slug, "sustainability-engagement-practitioner"))
    .limit(1);
  assert.ok(badge, "Sustainability Engagement Practitioner badge must exist");
  assert.equal(badge.name, "Sustainability Engagement Practitioner");

  // Verify system seed marker was recorded
  const [seedMarker] = await db
    .select()
    .from(systemSeedsTable)
    .where(eq(systemSeedsTable.name, "employee-sustainability-engagement-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker employee-sustainability-engagement-v2 must be recorded");
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "employee-sustainability-engagement-v2"));
