...
// Thorough cleanup of Course 21 data, recommendations, and orphans
async function cleanUpCourse21() {
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "employee-sustainability-engagement-v1"));

  // Reset Course 20 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-20"));

  const c21s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-21"));
  const c21Ids = c21s.map(c => c.id);
  if (c21Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c21Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c21Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c21Ids));
  }
}

test("Course 21 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse21();

  try {
    await db.transaction(async (tx) => {
      // 0. Set up prerequisite courses (ELH-12 and ELH-20) if they do not exist
      ...
      let c20 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-20")
      });
      if (!c20) {
        [c20] = await tx.insert(coursesTable).values({
          courseCode: "ELH-20",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 20",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 21 must be created");
      assert.equal(course.slug, "building-employee-engagement-in-sustainability");
      assert.equal(course.title, "Building Employee Engagement in Sustainability");
      ...
      assert.equal(badge.code, "COURSE_ELH_21_COMPLETE");

      // Verify prerequisites (ELH-12 and ELH-20)
      ...
      assert.ok(prereqIds.includes(c20.id), "Prerequisite should include Course 20");

      // Verify Course 20 recommendation link points to Course 21
      const [c20Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c20.id))
        .limit(1);
      assert.equal(c20Post.recommendedNextCourseId, course.id, "Course 20 should recommend Course 21");
...
test("Course 21 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse21();
  ...
  try {
    ...
    let c20 = await db.query.coursesTable.findFirst({ where: eq(coursesTable.courseCode, "ELH-20") });
    if (!c20) {
      [c20] = await db.insert(coursesTable).values({
        courseCode: "ELH-20",
        slug: "sustainability-roles-responsibilities-and-accountability",
        title: "Sustainability Roles, Responsibilities and Accountability",
        level: "Applied Workplace Practice",
    ...
    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-21")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "sustainability-engagement-facilitator")).limit(1);

    const [employee] = await db.insert(employeesTable).values({
      name: "Test Preserved User",
      email: "preserve21@ecolearn.mu",
...
test("Course 21 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse21();
  ...
  try {
    const mockTx = async (callback: any) => {
      return originalTransaction.call(db, async (tx: any) => {
        await callback(tx);
        throw new Error("Forced transaction failure for testing Course 21 rollback");
      });
    };
    ...
    assert.equal(err.message, "Forced transaction failure for testing Course 21 rollback");
    didThrow = true;
  } finally {
  ...
  assert.ok(!course, "Course 21 record should not exist because the transaction rolled back");
});
