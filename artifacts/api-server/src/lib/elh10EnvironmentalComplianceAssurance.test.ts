...
describe("Sprint 11J — ELH-10 Environmental Compliance Assurance & Regression Suite", () => {
  test("1. Course ELH-10 exists with correct metadata and learning objectives", async () => {
    ...
    assert.ok(course, "ELH-10 course record must exist");
  ...
  test("2. ELH-10 lessons contain required interactive scenarios and compliance hierarchy blocks", async () => {
    ...
    assert.equal(lessons.length, 6, "ELH-10 must contain 6 lessons");
    ...
    assert.ok(totalScenarios >= 3, "ELH-10 must contain at least 3 decision scenarios");
  });

  test("3. ELH-10 quiz questions exhibit zero 100% Position 1 correct-answer bias", async () => {
    ...
    assert.equal(questions.length, 5, "ELH-10 quiz must contain 5 scored questions");
