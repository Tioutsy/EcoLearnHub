...
test("Course 5 Seeding & Integrity Unit Tests", async () => {
  // 1. Initial seed execution
  console.log("- Running Course 5 seeder...");
  ...
  assert.ok(course, "Course 5 must exist");
  assert.equal(course.slug, "sustainable-procurement");
  assert.equal(course.passingScore, 80);
  assert.equal(course.status, "published");
  assert.equal(course.title, "Sustainable Procurement");

  // Verify exactly 6 lessons
  const lessons = await db
    .select()
    .from(lessonsTable)
    .where(eq(lessonsTable.courseId, 5));
  ...
  const quizQuestions = await db
    ...
    .where(eq(quizQuestionsTable.courseId, 5));
  assert.equal(
  ...
  const [badge] = await db
    ...
    .limit(1);
  assert.ok(badge, "Responsible Purchasing Contributor badge must exist");
  assert.equal(badge.name, "Responsible Purchasing Contributor");

  // Verify system seed marker was recorded
  const [seedMarker] = await db
    .select()
    .from(systemSeedsTable)
    .where(eq(systemSeedsTable.name, "sustainable-procurement-v2"))
    .limit(1);
  assert.ok(seedMarker, "Seed marker sustainable-procurement-v2 must be recorded");
  ...
  await db.delete(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, 5));
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "sustainable-procurement-v2"));
