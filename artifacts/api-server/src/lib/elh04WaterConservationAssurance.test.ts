...
describe("Sprint 11D — ELH-04 Water Conservation Assurance & Regression Suite", () => {
  test("1. Course ELH-04 exists with correct metadata and learning objectives", async () => {
    ...
    assert.ok(course, "ELH-04 course record must exist");
  ...
  test("2. ELH-04 lessons contain required interactive scenarios and hygiene boundary warnings", async () => {
    ...
    assert.equal(lessons.length, 6, "ELH-04 must contain 6 lessons");
    ...
    assert.ok(totalScenarios >= 3, "ELH-04 must contain at least 3 decision scenarios");
  });

  test("3. ELH-04 quiz questions exhibit zero 100% Position 1 correct-answer bias", async () => {
    ...
    assert.equal(questions.length, 5, "ELH-04 quiz must contain 5 scored questions");
