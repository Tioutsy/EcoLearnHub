...
// Thorough cleanup of Course 15 data, recommendations, and orphans
async function cleanUpCourse15() {
  ...
  await db.delete(systemSeedsTable).where(eq(systemSeedsTable.name, "workplace-sustainability-team-v1"));

  // Reset Course 14 recommendation link unconditionally
  await db.update(coursesTable)
    .set({ recommendedNextCourseId: null })
    .where(eq(coursesTable.courseCode, "ELH-14"));

  const c15s = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-15"));
  const c15Ids = c15s.map(c => c.id);
  if (c15Ids.length > 0) {
    await db.delete(lessonsTable).where(inArray(lessonsTable.courseId, c15Ids));
    await db.delete(quizQuestionsTable).where(inArray(quizQuestionsTable.courseId, c15Ids));
    await db.delete(coursesTable).where(inArray(coursesTable.id, c15Ids));
  }
}

test("Course 15 Seeding & Integrity Unit Tests", async () => {
  await cleanUpCourse15();

  try {
    await db.transaction(async (tx) => {
      // 0. Set up prerequisite courses (ELH-12 and ELH-14) if they do not exist
      ...
      let c14 = await tx.query.coursesTable.findFirst({
        where: eq(coursesTable.courseCode, "ELH-14")
      });
      if (!c14) {
        [c14] = await tx.insert(coursesTable).values({
          courseCode: "ELH-14",
          ...
          status: "published",
          isPublished: true,
          description: "Prerequisite Course 14",
          categoryId: 1,
        }).returning();
      }
      ...
      assert.ok(course, "Course 15 must be created");
      ...
      assert.equal(badge.code, "COURSE_ELH_15_COMPLETE");

      // Verify prerequisites (ELH-12 and ELH-14)
      ...
      assert.ok(prereqIds.includes(c14.id), "Prerequisite should include Course 14");

      // Verify Course 14 recommendation link points to Course 15
      const [c14Post] = await tx
        .select()
        .from(coursesTable)
        .where(eq(coursesTable.id, c14.id))
        .limit(1);
      assert.equal(c14Post.recommendedNextCourseId, course.id, "Course 14 should recommend Course 15");
...
test("Course 15 Learner Data Preservation Unit Tests", async () => {
  await cleanUpCourse15();
  ...
  try {
    ...
    let c14 = await db.query.coursesTable.findFirst({ where: eq(coursesTable.courseCode, "ELH-14") });
    if (!c14) {
      [c14] = await db.insert(coursesTable).values({
        courseCode: "ELH-14",
        slug: "setting-departmental-sustainability-goals",
        title: "Setting Departmental Sustainability Goals",
        level: "Applied Workplace Practice",
    ...
    // 1. Run seeder initially to create Course 15
    await ensureWorkplaceSustainabilityTeamCourse();

    const [course] = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-15")).limit(1);
    const [lesson] = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course.id)).limit(1);
    const [badge] = await db.select().from(badgeDefinitionsTable).where(eq(badgeDefinitionsTable.slug, "workplace-sustainability-team-builder")).limit(1);
...
test("Course 15 Transactional Rollback Atomicity Unit Tests", async () => {
  await cleanUpCourse15();
  ...
  // 4. Verify that Course 15 is not left partially seeded in the physical DB
  const course = await db.query.coursesTable.findFirst({
    where: eq(coursesTable.courseCode, "ELH-15")
  });
  assert.ok(!course, "Course 15 record should not exist because the transaction rolled back");
});
