...
const TEST_USER_ID = "c21_e2e_user";
const TEST_EMAIL = "c21-e2e@ecolearn.mu";
...
async function cleanDb() {
  ...
  const c21 = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-21")).limit(1);
  if (c21[0]) {
    await db.delete(courseCommitmentsTable).where(eq(courseCommitmentsTable.courseId, c21[0].id));
  }
...
test("Course 21 Full E2E Integration, Access Control, and Prerequisites Verification", async () => {
  ...
  try {
    ...
    // 1. Resolve Course 21 in the DB
    const c21Rows = await db.select().from(coursesTable).where(eq(coursesTable.courseCode, "ELH-21"));
    assert.equal(c21Rows.length, 1, "Course 21 does not exist in DB");
    const course21 = c21Rows[0];
    const course21Id = course21.id;

    assert.equal(course21.courseCode, "ELH-21", "Course 21 code should be ELH-21");
    assert.equal(course21.level, "Applied Workplace Practice", "Course 21 level should be Applied Workplace Practice");

    const c21Lessons = await db.select().from(lessonsTable).where(eq(lessonsTable.courseId, course21Id));
    assert.equal(c21Lessons.length, 6, "Course 21 should have exactly 6 lessons");

    const c21Questions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course21Id));
    assert.equal(c21Questions.length, 8, "Course 21 should have exactly 8 quiz questions");

    // 2. Ineligible Learner Access Control Verification
    let enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course21Id }),
    });
    assert.equal(enrollRes.status, 403, "Expected 403 Forbidden when enrolling without prerequisites");

    const courseDetailRes = await fetch(`${API_BASE}/courses/${course21Id}`, { headers: HEADERS });
    ...
    const quizRes = await fetch(`${API_BASE}/courses/${course21Id}/quiz`, { headers: HEADERS });
    ...
    const answersDummy = c21Questions.map((q) => ({
      questionId: q.id,
      selectedOption: 0,
    }));
    const submitResDummy = await fetch(`${API_BASE}/courses/${course21Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    // 3. Partially Eligible Learner (Complete ELH-12 but NOT ELH-20)
    ...
    assert.equal(c20Rows.length, 1, "Course 20 does not exist");
    const course20Id = c20Rows[0].id;
    ...
    // Verify Course 21 is STILL blocked
    ...
    // 4. Meet Remaining Prerequisites (Complete Course 20)
    await db.insert(enrollmentsTable).values({
      userId: TEST_USER_ID,
      employeeId: employee.id,
      courseId: course20Id,
    ...
    // Step 5a. Enrolling in Course 21 -> expect 201 Created
    enrollRes = await fetch(`${API_BASE}/enrollments`, {
      method: "POST",
      headers: HEADERS,
      body: JSON.stringify({ courseId: course21Id }),
    });
    assert.equal(enrollRes.status, 201, "Expected 201 Created for Course 21 enrollment");
    ...
    const eligibleCourseDetailRes = await fetch(`${API_BASE}/courses/${course21Id}`, { headers: HEADERS });
    ...
    for (const lesson of c21Lessons) {
    ...
    const dbQuestions = await db.select().from(quizQuestionsTable).where(eq(quizQuestionsTable.courseId, course21Id));
    const answersFail = dbQuestions.map((q) => ({
      questionId: q.id,
      selectedOption: (q.correctOption + 1) % 4,
    }));

    let submitRes = await fetch(`${API_BASE}/courses/${course21Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    const [badge] = await db
      ...
      .where(eq(badgeDefinitionsTable.slug, "sustainability-engagement-facilitator"))
      .limit(1);
    assert.ok(badge, "Badge Sustainability Engagement Facilitator must exist in DB");
    ...
    submitRes = await fetch(`${API_BASE}/courses/${course21Id}/quiz/submit`, {
      method: "POST",
      headers: HEADERS,
    ...
    assert.equal(badgeAwards.length, 1, "Should have awarded exactly 1 badge for Course 21 completion");
